...
function mapEmployee(row: any) {
  return {
    id: row.id, employerId: row.employer_id, firstName: row.first_name,
    lastName: row.last_name, dateOfBirth: row.date_of_birth, nationality: row.nationality,
    permitType: row.permit_type, avsNumber: row.avs_number, type: row.type,
    grossHourlyRate: row.gross_hourly_rate, hoursPerWeek: row.hours_per_week,
...
employees.get('/', (c) => {
  ...
  const rows = dbAll('SELECT * FROM employees WHERE employer_id = ? ORDER BY created_at DESC', [employerId]);
...
employees.get('/:id', (c) => {
  const userId = (c as any).get('userId') as string;
  const employerId = getEmployerId(userId);
  const row = dbGet('SELECT * FROM employees WHERE id = ? AND employer_id = ?', [c.req.param('id'), employerId]);
...
employees.post('/', async (c) => {
  ...
  const user = dbGet('SELECT plan FROM users WHERE id = ?', [userId]);
  const countRow = dbGet('SELECT COUNT(*) as count FROM employees WHERE employer_id = ? AND status = ?', [employerId, 'active']);
  ...
  if (existingCount >= maxEmployees) {
    return c.json({
      success: false,
      error: `Votre formule permet ${maxEmployees} employé(s) actif(s) maximum.`,
    }, 403);
  }
  ...
  dbRun(
    'INSERT INTO employees (id, employer_id, first_name, last_name, date_of_birth, nationality, permit_type, avs_number, type, gross_hourly_rate, hours_per_week, start_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
  ...
  const created = dbGet('SELECT * FROM employees WHERE id = ?', [id]);
...
employees.put('/:id', async (c) => {
  ...
  const existing = dbGet('SELECT id FROM employees WHERE id = ? AND employer_id = ?', [employeeId, employerId]);
  if (!existing) return c.json({ success: false, error: 'Employé non trouvé' }, 404);

  const body = await c.req.json();
  const { firstName, lastName, dateOfBirth, nationality, permitType, avsNumber, type, grossHourlyRate, hoursPerWeek, startDate, endDate, status } = body;

  dbRun(
    'UPDATE employees SET first_name=?, last_name=?, date_of_birth=?, nationality=?, permit_type=?, avs_number=?, type=?, gross_hourly_rate=?, hours_per_week=?, start_date=?, end_date=?, status=? WHERE id=?',
